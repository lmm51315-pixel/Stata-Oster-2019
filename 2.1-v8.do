*! Version 1.0.4  31Jan2026
*! Author: [Ma Junhao]
*! Implements Oster (2019) Stability Test with PSACALC mathematical core
*! Modification: Removed internal bootstrap. Compatible with Stata 'bootstrap' prefix.
...
program define coefstability, rclass sortpreserve
	...
	* ===================== PATCH: sync locals for display/return =====================
	...
	* ===================== PATCH: safe fallback (beta only) =====================
	* 只在Mata失败时补beta_star；delta_star完全由Mata的cs_bound给出，不要用简化式覆盖
...
program define _get_vars_consistent, rclass
    ...
    // ---------------------------------------------------------
    // sigma_yy: PSACALC对齐
    // - xtreg: 用within口径Var(y - ybar_i)
    // - 其他模型: 用FULL模型估计样本上的Var(y)
    // 重要：中途跑的辅助回归会污染e()，必须restore回est_full
    // ---------------------------------------------------------
	...
	else if "`cmd_full_now'" == "areg" {
		local av_f "`e(absvar)'"
		capture quietly areg `depvar_act' `__kfix_y' `wgt' if `__touse', absorb(`av_f')
		if _rc == 0 {
	...
	* 兜底：万一上述失败，至少给overall Var(y)
    ...
    // Patch4: 全部用 if e(sample)
    // Patch3: reghdfe内部回归删掉keepsingletons
    // ---------------------------------------------------------
    ...
    else if "`cmd_base'" == "areg" {
        local av_b "`e(absvar)'"
        if "`base_controls'" != "" {
            capture quietly areg `treatment_var' `base_controls' `wgt' if `__touse', absorb(`av_b')
        }
        else {
            capture quietly areg `treatment_var' `__kfix' `wgt' if `__touse', absorb(`av_b')
        }
        if _rc != 0 {
            return scalar sigma_xx = .
            return scalar t_x      = .
    ...
    else if "`cmd_full'" == "areg" {
        local av_f "`e(absvar)'"
        if "`full_controls'" != "" {
            capture quietly areg `treatment_var' `full_controls' `wgt' if `__touse', absorb(`av_f')
        }
        else {
            capture quietly areg `treatment_var' `__kfix2' `wgt' if `__touse', absorb(`av_f')
        }
        if _rc != 0 {
            return scalar t_x = .
            exit
...
program define _get_r2_new, rclass
    ...
    else if "`cmdname'" == "areg" {
        return scalar r2 = e(r2)
        exit
    }
    else if "`cmdname'" == "reghdfe" {
		return scalar r2 = e(r2)
		if missing(r(r2)) return scalar r2 = e(r2_within)
		exit
	}
