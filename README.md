...
本文详细介绍了该命令的语法结构、参数设定及推断功能，并通过复现 *Energy Economics* 量篇近期文献（Acheampong & Said, 2024; Cepni et al., 2024；）和*Management Science*的一篇近期文献（Bao et al.，2025），展示了其在截面与面板数据分析中的应用。`coefstability` 为研究者提供了一个便捷、严谨的工具，以量化评估实证结果的稳健性。

### 内容说明

- data：存放本文复现案例所使用的数据，文件名对应原文名称。
- myado：存放自定义命令 `coefstability` ，以及运行本文 dofile 所需的外部命令。
- Orig_dofile：存放所复现三篇文章作者提供的原始资料。
